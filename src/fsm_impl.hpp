...
Generation date: 2026-05-06 13:08:56
...
state_t do_init(T &data) {
  ...
  try{
    ...
    cerr << "Program loaded: " << endl << *data.program << endl;
...
state_t do_idle(T &data) {
  ...
  /* Wait for user input */
  cerr << "Press <SPACE> to run, Z to go to 0, R to reload program, Q to quit" << endl;
  ...
  switch(key) {
    ...
    case 'R':
      try{
        ...
        cerr << "Program reloaded:" << endl << *data.program << endl;
      } catch(const exception &e) {
...
state_t do_rapid_motion(T &data) {
  ...
  cerr << fg::cyan << "Rapid motion block: " << fg::reset << b << endl;
  next_state = cncpp::STATE_LOAD_BLOCK;
